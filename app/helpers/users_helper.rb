    ...
    freind_requests.each do |friend_request|
      str +="<li>
        <a href='#'>
          ...
          <button class='btn btn-success btn-xs'><i class='fa fa-fw fa-check-circle'></i>&nbsp;Accept</button>&nbsp;&nbsp;<button class='btn btn-danger btn-xs'><i class='fa fa-fw fa-times-circle'></i>&nbsp;Reject</button></div>
          </br><span><small>#{time_ago_in_words(User.find(1).pending_received_friend_requests.first.created_at)}&nbsp;ago</small></span>
        </a>
      </li>".html_safe()
    end
    str
